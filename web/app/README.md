...
the Web Serial API**. There is **no backend server** — the operator just opens a
URL (or serves the static build) in Chrome/Edge. The Elixir/Phoenix app under the
repo root remains the side-by-side reference.
