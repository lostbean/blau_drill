...
import blau_drill/ui/model.{
  ...
  Disconnected, DisconnectDevice, Done, Drill, DrillMode, DrlPicked, DryRun,
  ...
  HaveProgress, HaveSummary, HaveTransform, Head, HoleDone, Jog, Jogging,
  JumpTo, Load, LoadSample, Model, NavStage, NewBoard, NoBitChange, NoBoard,
  NoBoardModel, NoDiagnostic, NoHeadPos, NoJob, NoProgress, NoSummary, NoTransform,
  OutlinePicked, ParseBoard, Progress, Reconnect, Recapture, RedoAlignment,
  RealBackend, Release, ResetDefaults, ResetView, RestartAlignment, ResumeDrilling,
  RunDryRun, SelectBackend, SelectCategory, SelectFile, SelectOutline,
  SetConfigField, SetCurrentTarget, SetJogStep, Settings, SimBackend, StageAlign,
  StageDone, StageDrill, StageDryRun, StageLoad, StartRegistering, Summary,
...
fn apply_controller(model: Model, cmsg: controller.ControllerMsg) -> #(Model, Effect(Msg)) {
...
fn fold_events(model: Model, events: List(printer.Event)) -> #(Model, Effect(Msg)) {
...
fn fold_event(model: Model, ev: printer.Event) -> #(Model, Effect(Msg)) {
  case ev {
    ...
    printer.Progress(sent, total, _line) -> noeff(apply_progress(model, sent, total))
...
fn drl_picked(model: Model, res: Result(String, String)) -> #(Model, Effect(Msg)) {
  case res {
    Ok(text) ->
      noeff(Model(..model, pending_drl: text, file_selected: True, upload_error: ""))
    // A cancelled picker is silent; a real read error surfaces.
    Error("cancelled") -> noeff(model)
    Error(reason) -> noeff(Model(..model, upload_error: "File error: " <> reason))
  }
}

fn outline_picked(model: Model, res: Result(String, String)) -> #(Model, Effect(Msg)) {
  case res {
    Ok(text) ->
      noeff(Model(..model, pending_edge_cuts: text, outline_file: "Edge.Cuts.svg"))
    Error("cancelled") -> noeff(model)
    Error(reason) -> noeff(Model(..model, upload_error: "Outline error: " <> reason))
  }
}
...
fn connect_device(model: Model) -> #(Model, Effect(Msg)) {
  ...
  let eff = controller.connect(model.controller, baud) |> effect.map(ControllerEvent)
  #(model, eff)
}
...
fn request_where_effect() -> Effect(Msg) {
  use dispatch <- effect.from
  promise.resolve(Nil)
  |> promise.map(fn(_) { dispatch(ControllerEvent(controller.Issue(printer.Where))) })
  Nil
}
            ...
            Ok(#(bx, by)) -> {
              ...
              case already {
                ...
                False -> {
                  ...
                  let captured =
                    list.append(model.captured, [Fiducial(bx, by, idx, Captured)])
                  let captures =
    ...
    HaveJob(j) ->
      case job.can(j, job.RunDryRunE), j.alignment {
        True, Some(al) -> {
          ...
          let cfg = config.GcodeConfig(..model.applied_config, mode: config.DryRun)
          ...
          let m =
            Model(
              ...
              progress: HaveProgress(Progress(drilled: 0, total: total, mode: DryRunMode)),
            )
    ...
    HaveJob(j) ->
      case job.can(j, job.ConfirmRegistrationE), j.alignment {
        True, Some(al) -> {
          ...
          let cfg = config.GcodeConfig(..model.applied_config, mode: config.Drill)
          let program = gcode_program.build(j.board, al, cfg)
          let total = list.length(j.board.holes)
          ...
          let #(bit_change, bit_label, changes) = case program.tool_order {
            [_first, second, ..] -> {
              ...
              #(
                ...
                fmt_mm(tool_diameter(j.board, first_tool(program.tool_order))) <> "mm",
          ...
          let m =
            Model(
              ...
              job: j2,
              board: reset_hole_status(model.board),
              progress: HaveProgress(Progress(drilled: 0, total: total, mode: DrillMode)),
...
fn reconnect(model: Model) -> #(Model, Effect(Msg)) {
  ...
  let m2 = Model(..m1, screen: Align, progress: NoProgress, bit_change: NoBitChange)
  #(m2, eff1)
}
  ...
  case model.progress, model.job {
    HaveProgress(p), HaveJob(j) -> {
      ...
      Model(
        ..model,
        board: board,
        progress: HaveProgress(Progress(..p, drilled: holes_done, total: total_holes)),
...
fn reset_hole_status(board: model.BoardOpt) -> model.BoardOpt {
  case board {
    HaveBoard(b) -> {
      let holes = list.map(b.holes, fn(h) { model.Hole(..h, status: model.Pending) })
