...
pub fn command(state: PrinterState, cmd: Command) -> Step {
  case state, cmd {
    // ── connect / disconnect ────────────────────────────────────────────────
    Disconnected, Connect -> accepted(Idle(line_no: 0, pending: PendingNone), [], cmd)
...
pub fn feed(state: PrinterState, raw_line: String) -> Step {
  ...
  case state {
    ...
    Idle(line_no, PendingWhere) -> feed_where(state, line_no, line, fn(n, p) {
      Idle(line_no: n, pending: p)
    })
    Jogging(line_no, PendingWhere) -> feed_where(state, line_no, line, fn(n, p) {
...
fn feed_where(
  ...
  case protocol.parse_m114(line) {
    Ok(pos) ->
      Step(rebuild(line_no, PendingNone), [], [PositionUpdate(pos)])
