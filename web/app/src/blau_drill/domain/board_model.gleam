...
fn parse_drl_body(drl: String) -> Result(#(ToolTable, List(Hole)), ParseError) {
  ...
  case list.contains(lines, "M48") {
    ...
    True ->
      scan(
        lines,
        ScanState(tools: dict.new(), holes: [], active: None),
      )
  }
}
        ...
        False ->
          case tool_def(line) {
            Ok(#(tool, diameter)) ->
              scan(
                rest,
                ScanState(..state, tools: dict.insert(state.tools, tool, diameter)),
              )
            Error(_) ->
...
fn check_page_coordinates(holes: List(Hole)) -> Result(Nil, ParseError) {
  ...
  case oversized != [], origin_offset >. max_origin_offset_mm {
    True, _ ->
      Error(AbsolutePageCoordinates(CoordinateOverBedSize(
        threshold_mm: max_plausible_coord_mm,
        sample: list.take(oversized, 3),
      )))
    False, True ->
      Error(AbsolutePageCoordinates(BboxFarFromOrigin(
