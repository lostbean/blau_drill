...
import blau_drill/ui/model.{
  ...
  DisconnectDevice, Disconnected, Drill, DryRun, Done, Faulted, GoToSettings,
  Idle, Jogging, Load, NavStage, RealBackend, SelectBackend, Settings, SimBackend,
...
pub fn fault_banner() -> Element(model.Msg) {
  h.div([a.class("fault-banner")], [
    ...
    h.button(
      [a.class("fault-reconnect"), a.attribute("type", "button"), event.on_click(model.Reconnect)],
      [h.text("Reconnect")],
    ),
  ])
}
