...
import blau_drill/ui/model.{
  ...
  ConfirmRegistration, Connection, Defaults, Energize, Fit,
  Jog, Jogging, MotionLimits, NewBoard, ParseBoard, Recapture, RedoAlignment,
  Release, RestartAlignment, ResumeDrilling, RunDryRun, SelectCategory,
...
fn motors_panel(online: Bool) -> Element(model.Msg) {
  ...
  h.div([a.class("panel")], [
    ...
    h.p([a.class("panel-hint")], [h.text("Enable motors to unlock jog controls.")]),
  ])
}
...
pub fn drill(model: Model) -> Element(model.Msg) {
  let #(pct, drilled, total) = case model.progress {
    model.HaveProgress(p) -> #(progress_pct(p.drilled, p.total), p.drilled, p.total)
...
fn telemetry_row(label: String, value: String, color: String) -> Element(model.Msg) {
  ...
  case model.summary {
    model.HaveSummary(s) ->
      h.div([a.class("summary-grid")], [
        summary_cell("Total Holes", int.to_string(s.total_holes), "secondary", False),
    ...
    card("Serial Port", [
      ...
      h.div([a.class("toggle-row")], [
        ...
        h.div([a.class("toggle-text")], [
          ...
          h.span([a.class("sub")], [
            h.text("Establish serial connection automatically when the system boots."),
          ]),
        ]),
      ]),
    ]),
...
fn number_field(name: String, label: String, value: String) -> Element(model.Msg) {
  h.div([a.class("field")], [
    h.label([a.attribute("for", field_id(name))], [h.text(label)]),
    h.input([
      a.id(field_id(name)),
