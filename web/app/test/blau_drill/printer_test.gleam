...
import blau_drill/control/printer.{
  type Command, type Event, type PrinterState, type Step, Accepted, Busy, Connect,
  Energize, Faulted, Faulting, Halt, Jog, MoveTo, NotEnergized, PositionUpdate,
  Progress, PulseSpindle, Reconnect, Recovered, Refused, Release, Stream, Where,
  X, Y,
}
...
pub fn move_to_in_idle_writes_nothing_test() {
  ...
  step.writes |> should.equal([])
  has_event(step.events, fn(e) { e == Refused(MoveTo(10.0, 20.0), NotEnergized) })
  |> should.be_true
}
...
fn run_stream_state(state: PrinterState, program: List(String)) -> PrinterState {
  let s0 = printer.command(state, Stream(program))
