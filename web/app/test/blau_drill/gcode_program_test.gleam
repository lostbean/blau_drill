...
pub fn drill_mode_xy_safe_test() {
  let p = gcode_program.build(board_from_fixture(), xmirror_alignment(), cfg(Drill))
  xy_only_when_safe(p) |> should.be_true
}

pub fn dry_run_mode_xy_safe_test() {
  let p = gcode_program.build(board_from_fixture(), xmirror_alignment(), cfg(DryRun))
  xy_only_when_safe(p) |> should.be_true
}
...
fn parse_z_of_g1(line: String) -> Option(Float) {
  let assert Ok(re) = regexp.from_string("^\\s*[Gg]0?1\\s+Z(-?\\d+(?:\\.\\d+)?)")
  case regexp.scan(re, line) {
    [match, ..] ->
      case match.submatches {
...
pub fn tool_change_pauses_test() {
  ...
  list.filter(p.lines, fn(l) { regexp.check(m6, l) }) |> list.length
  |> should.equal(5)
  // Touch-off M0 + 5 tool-change M0 = 6.
  list.filter(p.lines, fn(l) { regexp.check(m0, l) }) |> list.length
  |> should.equal(6)
}
...
pub fn drill_zdepths_test() {
  ...
  list.filter(p.lines, fn(l) { regexp.check(rt, l) }) |> list.length
  |> should.equal(130)
...
pub fn tool_structure_test() {
  ...
  list.filter(p.lines, fn(l) { regexp.check(feed, l) }) |> list.length
  |> should.equal(5)
...
fn round5(v: Float) -> Float {
  ...
  sign *. int.to_float(float.round(float.absolute_value(v) *. 100_000.0))
  /. 100_000.0
}
...
pub fn defaults_to_dry_run_test() {
  let p =
    gcode_program.build(board_from_fixture(), xmirror_alignment(), config.default())
