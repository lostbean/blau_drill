...
pub fn identity_maps_points_to_themselves_test() {
  ...
  pts
  |> assert_each(fn(p) { assert_point(t2d.apply(t2d.identity(), p), p, delta) })
}
...
fn invertible(angle: Float, sx: Float, sy: Float, tx: Float, ty: Float) {
  ...
  let sin = sin_(angle)
  let rotation =
    Transform2D(a: cos, b: float.negate(sin), c: sin, d: cos, tx: 0.0, ty: 0.0)
...
pub fn zero_determinant_transforms_are_singular_test() {
  ...
  cases
  |> assert_each(fn(t) { t2d.invert(t) |> should.equal(Error(t2d.Singular)) })
}
